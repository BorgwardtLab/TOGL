...
def parallel_persistence_routine(filtered_v_, data: Data, cycles = False):
    ...
    if cycles:
        persistence1 = torch.zeros((len(filtered_e),2, num_filtrations), device = filtered_v_.device) 
    ...
    for edge_index, edge_weight in zip(e_indices,filtered_e):
        ...
        persistence[younger[correct_mask],0,np.arange(num_filtrations)[correct_mask]] = values_younger[correct_mask]
        persistence[older[flipped_mask],0,np.arange(num_filtrations)[flipped_mask]] = values_older[flipped_mask]

        persistence[younger[correct_mask],1,np.arange(num_filtrations)[correct_mask]] = edge_weight[correct_mask]
        persistence[older[flipped_mask],1,np.arange(num_filtrations)[flipped_mask]] = edge_weight[flipped_mask]
        
        if cycles:
            persistence1[edge_index[equal_mask],0,np.arange(num_filtrations)[equal_mask]] = filtered_e_[edge_index,np.arange(num_filtrations)][equal_mask]
            persistence1[edge_index[equal_mask],1,np.arange(num_filtrations)[equal_mask]] = unpaired_values[equal_mask]
    ...
    for root in uf.roots():
        ...
        persistence[root,0,np.arange(num_filtrations)][cycle_mask] = filtered_v_[root,np.arange(num_filtrations)][cycle_mask]
        persistence[root,1,np.arange(num_filtrations)][cycle_mask] = unpaired_values[cycle_mask]
