...
def main(args):
    ...
    wandb_logger = WandbLogger(name=f"GCN_{args.dataset}", project="topo_gnn", entity="topo_gnn", tags=[
                               args.dataset], log_model=True)
    ...
    data = topodata.TUGraphDataset(
        args.dataset, batch_size=32, add_node_degree=add_node_degree, fold=args.fold)
    data.prepare_data()
