...
class TopologyLayer(torch.nn.Module):
    ...
    def compute_coord_activations(self,persistences,batch,dim1 = False):
        ...
        coord_activations = [self.compute_coord_fun(persistence,dim1 = dim1) for persistence in persistences]
    ...
    def forward(self,x, batch):
        ...
        if self.dim1:        
            ...
            graph_activations1 = self.collapse_dim1(coord_activations1,persistence1_mask, batch.__slices__["edge_index"]) # returns a vector for each graph
        else:
...
class FiltrationGCNModel(pl.LightningModule):
    ...
    def __init__(self, hidden_dim, filtration_hidden, num_node_features, num_classes, num_filtrations, num_coord_funs,dim1 = False, num_coord_funs1 = None, lr = 0.001, dropout_p = 0.2):
        ...
        if dim1:
            cycles_dim = num_filtrations * np.array(list(num_coord_funs1.values())).sum()
        else:
            cycles_dim = 0
